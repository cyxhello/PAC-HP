...
def training_loop(num_epochs, dataloader, netG, netD, device, criterion, nz, optimizerG, optimizerD, fixed_noise, out):
    ...
    for epoch in range(num_epochs):
        for i, data in enumerate(dataloader, 0):
            ...
            #             save_image(fake[j], 'gen/' + params['run'] +
            #                     '_' + str(i) + '_' + str(j) + '_img.png')
        p, s, h, ms, md = compute_metrics(real=next(iter(dataloader)), fakes=img_list_only, size=64, out='')
        ...
        if epoch % 30 == 0 or epoch == num_epochs-1:
            torch.save(netG.state_dict(), 'checkpoints/single_' + str(epoch) +'.pth')
